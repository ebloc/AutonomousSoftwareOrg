...
import {Context} from "./ERC20/Context.sol";
...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Errors {
     ...
     *
     */
    function _setDataHash(uint256 tokenId, bytes32 _dataHash) internal virtual {
        require(_exists(tokenId), "ERC721Metadata: Hash set of nonexistent token");
        _hashIDs[tokenId] = _dataHash;
    }
    ...
    function tokenHash(uint256 tokenId) external view returns (bytes32) {
        ...
        bytes32 _dataHash = _hashIDs[tokenId];
...
contract ResearchCertificate is ERC721 {
    ...
    constructor() public ERC721("ResearchObjectCertification", "ROC") {}

    mapping(bytes32 => uint256) public dataHashToTokenIndex;
    bytes32[] dataHashArray;

    function createCertificate(address recipient, bytes32 dataHash)
        public
        returns (uint256)
    {
        if (dataHashToTokenIndex[dataHash] == 0) {
            _tokenIds.increment();
            uint256 newItemId = _tokenIds.current();
            _mint(recipient, newItemId);
            _setDataHash(newItemId, dataHash);
            dataHashToTokenIndex[dataHash] = newItemId;
            dataHashArray.push(dataHash);
            return newItemId;
        }
        else {
            return dataHashToTokenIndex[dataHash];
        }
    }
